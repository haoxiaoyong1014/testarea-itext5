...
/**
 * <p>
 * This is a copy of {@link com.itextpdf.text.pdf.pdfcleanup.PdfCleanUpRegionFilter}
...
class PdfCleanUpRegionFilter extends RenderFilter {
    ...
    protected Path filterStrokePath(Path sourcePath, Matrix ctm, float lineWidth, int lineCapStyle,
        ...
        ClipperOffset offset = new ClipperOffset(miterLimit, StrictPdfCleanUpProcessor.arcTolerance * StrictPdfCleanUpProcessor.floatMultiplier);
    ...
    private static List<Point2D> convertToFloatPoints(List<LongPoint> points) {
        ...
        for (LongPoint point : points) {
            convertedPoints.add(new Point2D.Float((float) (point.getX() / StrictPdfCleanUpProcessor.floatMultiplier),
                                                  (float) (point.getY() / StrictPdfCleanUpProcessor.floatMultiplier)));
        }
